...
function applySpecialConfig(type, entity) {
  return new Promise(function (resolve, reject) {
    if (type === "/client") {
      ...
      if (entity.redirectURI === "set-automatically" && process.env.AGILE_HOST) {
        entity.redirectURI = "http://" + process.env.AGILE_HOST + ":8000/";
      } else if (entity.redirectURI === "set-automatically" && hostname) {
        entity.redirectURI = "http://" + hostname + ".local:8000/";
      }
      resolve(entity);
    } else {
      resolve(entity);
