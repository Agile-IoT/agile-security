...
TokenStorage.prototype.createTables = function (onInitFinished, error) {
  //TODO extend this to include the possibility of storing refresh tokens (token["refresh_token"])
  if (error) {
    ...
    this.storage.run("CREATE TABLE IF NOT EXISTS token (id TEXT PRIMARY KEY, cookie TEXT NOT NULL, token TEXT NOT NULL, scope TEXT, token_type TEXT, auth_type TEXT NOT NULL, user_id TEXT NOT NULL, expiration DATETIME DEFAULT 0, Timestamp DATETIME DEFAULT CURRENT_TIMESTAMP)",
      function (error) {
        if (error != null) {
...
TokenStorage.prototype.prepareStatements = function (onInitFinished, error) {
  ...
  var promisses = [
    new Promise(function (resolve, reject) {
      this.store_token_statement = this.storage.prepare("INSERT INTO token  (id , cookie , token , scope , token_type , auth_type,user_id, expiration ) VALUES(?, ?, ?, ?, ?, ?,?,?)", promisseHandle.bind(this, resolve, reject));
    }.bind(this)),
    ...
    new Promise((resolve, reject) => {
      this.get_token_by_cookie_id_statement = this.storage.prepare("SELECT id , cookie , token , scope , token_type , auth_type,user_id, expiration  from token WHERE cookie =?", promisseHandle.bind(this, resolve, reject));
    }),
    new Promise((resolve, reject) => {
      this.get_token_by_token_statement = this.storage.prepare("SELECT id , cookie , token , scope , token_type , auth_type,user_id, expiration  from token WHERE token =?", promisseHandle.bind(this, resolve, reject));
    }),
    new Promise((resolve, reject) => {
      this.get_token_by_user_id = this.storage.prepare("SELECT id , cookie , token , scope , token_type , auth_type,user_id, expiration  from token WHERE user_id =?", promisseHandle.bind(this, resolve, reject));
    })
  ]
...
TokenStorage.prototype.storeToken = function (token_id, cookie_id, auth_type, token, onStorageTokenFinished) {
  //console.log(JSON.stringify(arguments));
  this.store_token_statement.run(token_id, cookie_id, token['token'], token["scope"], token["token_type"], token["auth_type"], token["user_id"], token["expiration"], onStorageFinished.bind(this, onStorageTokenFinished));

  function onStorageFinished(onStorageTokenFinishedCallback, error) {
    if (error) {
      onStorageTokenFinishedCallback({
        "success": false,
...
TokenStorage.prototype.getTokenById = function (id, onStorageTokenFinishedCallback) {
  ...
  this.get_token_by_id.get(id, onStorageFinished.bind(this, onStorageTokenFinishedCallback));
}
