...
function loadStrategy(conf) {
  ...
  try {
    ...
    console.log('pam strategy configured as PAM');
    ...
    if (conf.hasOwnProperty("auth") && conf.auth.hasOwnProperty("local") && conf.auth.local.hasOwnProperty("fallback-user-no-pam")) {
      ...
      console.log('pam strategy neither configured as PAM nor to use  local users');
  ...
  function setToken(storage, username, auth_type, done) {
    ...
    var accessToken = tokens.uid(256);
    var token = {};
  ...
  connectionPoolPromisse(conf).then(function (storage) {
    try {
      ...
      passport.use('pam', new LocalStrategy(
      ...
      console.log('finished registering passport pam strategy');
    } catch (e) {
