...
function loadStrategy(conf){
    ...
    try {
        ...
        pam = require('authenticate-pam');
         ...
         if(conf.hasOwnProperty("auth") && conf.auth.hasOwnProperty("local") && conf.auth.hasOwnProperty("fallback-user-no-pam")){
            user =conf["auth"]["local"]["fallback-user-no-pam"];
         }
         else{
           throw new Error("Local authentication not configured properly");
         }
