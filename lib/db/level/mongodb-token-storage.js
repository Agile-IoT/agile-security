...
function getTokenById(id, callback) {
  ...
  that.tokens.findOne({token: id}, function (error, value) {
    ...
    if (error || value === null || !value) {
      callback({
        success: false,
        error: error && error.type === 'NotFoundError' ? createError(404, "id not found " + JSON.stringify(id) + error) : createError(500, "unexpected error while retrieving token with id " + JSON.stringify(id))
      });
    } else {
