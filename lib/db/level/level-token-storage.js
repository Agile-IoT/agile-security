...
TokenStorage.prototype.findByUserAndClientAndOauth2Type = function (username, auth_type, client_id, oauth2_flow, callback) {
  ...
  that.tokens.createReadStream()
    .on('data', function (data) {
      //var username = data.value.user_name;
      if (data.value.expiration !== null && data.value.expiration < t) {
        that.deleteTokenByToken(data.key, function (res) {
          console.log("token dropped for " + data.value.user_name + " - result : " + JSON.stringify(res));
        });
      } else {
        ...
        if (data.value.user_name === username && data.value.clientId === client_id && data.value.auth_type === auth_type && data.value.oauth2 === oauth2_flow) {
          console.log("adding token to the list " + JSON.stringify(data.value));
