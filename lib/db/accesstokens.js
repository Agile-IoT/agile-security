  ...
  connectionPoolPromisse(conf).then(function (storage) {
    storage.getTokenByToken(key, function (result) {
      if (result.hasOwnProperty("success") && result.success) {
        ...
        if (token.expiration === null || token.expiration > new Date().getTime()) {
          ...
          console.log('found EXPIRED!!!! token info  by. for user id ' + token.userID +
            ' and for client id ' + token.clientId +
            ' and with expiration ' + token.expiration +
            ' and scope ' + JSON.stringify(token.scope));
          console.log('removing  EXPIRED!!! token');
          storage.deleteTokenByToken(token.token, function (result) {
      ...
      storage.findByUserAndClientAndOauth2Type(username, auth_type, clientID, oauth2_flow, function (result) {
        ...
        if (result.hasOwnProperty("success") && result.success) {
          ...
          if (token.expiration === null || token.expiration > new Date().getTime()) {
            ...
            console.log('found EXPIRED!!! token info found by id. for  user ' + username + " auth " + auth_type + " client " + clientID + " oauth " + oauth2_flow);
            console.log('removing  EXPIRED!!! token');
            storage.deleteTokenByToken(token.token, function (result) {
              return done(null, null);
            });
...
var save = function (conf, tok, userID, clientID, token_type, scope, expiration, refreshToken, done) {
  connectionPoolPromisse(conf).then(function (storage) {
      ...
      token.refreshToken = refreshToken;
      token.user_name = ids.getUsername(token.userID);
      token.auth_type = ids.getAuthType(token.userID);
      token.oauth2 = null;
      internalSave(storage, tok, token, done);
    },
    function (error) {
...
module.exports = function (conf) {
  return {
    ...
    //save is used in the strategies. It must store client = null, so that the oauth2orize handler can update this later
    save: save.bind(this, conf), // save used from all the providers. They still don't know the oauth2 flow type nor the client id
    //these two methods update the token and find it including the client and oauth2 flow used
