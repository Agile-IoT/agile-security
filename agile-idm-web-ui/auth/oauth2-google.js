...
*/
function GoogleOauth2(app, conf, storage){
			...
			request.get(options, function(error, response, body){
				if (!error && response.statusCode == 200) {
					...
					if(response.statusCode == 401){
						result.error = "bad credentials for google authentication";
					}
					reject(result);
				}
				else{
	...
	function exchangeTokenByPrincipalFinished(res, token, result){
		if(result[0].success && result[0].data){
			...
			console.log('authenticated with Google as '+result[0].data.id);
			storage.storeToken(cookie_id, "google", token, onStorageTokenFinished.bind(this, res, cookie_id));
		}
		else{
